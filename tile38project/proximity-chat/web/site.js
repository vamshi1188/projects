...
let connectedWith = null; // Client ID of the connected user
let incomingRequest = null; // Client ID of the user requesting connection
...
// Load the map
// Using OpenStreetMap tiles instead of Mapbox (no token required)
...
map.on('load', function () {
    ...
    chatInput.addEventListener('keypress', function (ev) {
        ...
        if (ev.charCode == 13) {
            ...
            if (connected && message != '') {
                console.log(message)
                if (connectedWith) {
                    sendPrivate(connectedWith, 'Chat', { text: message });
                    // Update own chat UI
                    updateChat(me, message, true);
                } else {
                    // Broadcast (legacy behavior, or disable?)
                    // Let's keep broadcast for now if not connected, or maybe disable it?
                    // The requirement says "chat anonymously user open application enters his name and give location permison it will show available persons on the map if i click on person it will ask me to send request if he or she accepts both will connect and chat"
                    // This implies chat is ONLY when connected.
                    // But let's leave broadcast as a fallback or disable it.
                    // Let's disable broadcast to strictly follow "connect and chat".
                    alert("Click on a user to connect and chat!");
                }
                this.value = '';
    ...
    document.getElementById('accept-btn').addEventListener('click', function () {
        if (incomingRequest) {
            acceptConnection(incomingRequest);
    ...
    document.getElementById('reject-btn').addEventListener('click', function () {
        if (incomingRequest) {
            rejectConnection(incomingRequest);
            document.getElementById('request-modal').style.display = 'none';
            incomingRequest = null;
        }
    });
...
function createMarker(feature, anim) {
    ...
    el.addEventListener('click', function () {
        if (hideReady) {
            ...
            if (!connectedWith) {
                requestConnection(feature.id);
            } else if (connectedWith == feature.id) {
                // Already connected
            } else {
                alert("You are already connected with someone else. Disconnect first.");
            }
        }
    })
...
function drawConnection(ctx, marker, memarker) {
    // Only draw connection if this marker is the one we are connected with
    if (marker.feature.id != connectedWith) {
        return;
    }
    ...
    // Force full fade for connected line
    let fadeA = 1; // marker.fade*marker.nearbyFade;
    let fadeB = 1; // memarker.fade;
...
function sendPrivate(targetId, type, payload) {
    if (connected) {
        ws.send(JSON.stringify({
            ...
            subtype: type,
            payload: payload,
            feature: me // Include own feature for identification
        }));
    }
}
...
function requestConnection(targetId) {
    if (confirm("Send connection request?")) {
        sendPrivate(targetId, 'Request', {});
        alert("Request sent!");
    }
}

function acceptConnection(targetId) {
    connectedWith = targetId;
    sendPrivate(targetId, 'Accept', {});
    updateConnectionStatus(targetId);
}

function rejectConnection(targetId) {
    sendPrivate(targetId, 'Reject', {});
}

function disconnect() {
    if (connectedWith) {
        sendPrivate(connectedWith, 'Disconnect', {});
        connectedWith = null;
        updateConnectionStatus(null);
    }
}

function updateConnectionStatus(targetId) {
    const statusBox = document.getElementById('connection-status');
    const statusText = document.getElementById('connected-with');
    if (targetId) {
        statusBox.style.display = 'flex';
        let name = "Unknown";
        if (markers.has(targetId)) {
            name = markers.get(targetId).feature.properties.name || "User";
        }
        statusText.innerText = 'Connected with: ' + name;
    } else {
        statusBox.style.display = 'none';
        statusText.innerText = 'Connected with: None';
    }
}

function handlePrivate(msg) {
    const senderId = msg.feature.id;
    const senderName = msg.feature.properties.name || "Unknown User";

    switch (msg.subtype) {
        case 'Request':
            if (connectedWith) {
                // Auto-reject if already connected
                sendPrivate(senderId, 'Reject', { reason: "Busy" });
            } else {
                incomingRequest = senderId;
                document.getElementById('request-text').innerText = senderName + " wants to connect";
                document.getElementById('request-modal').style.display = 'flex';
            }
            break;
        case 'Accept':
            connectedWith = senderId;
            updateConnectionStatus(senderId);
            alert(senderName + " accepted your request!");
            break;
        case 'Reject':
            alert(senderName + " rejected your request.");
            break;
        case 'Disconnect':
            if (connectedWith == senderId) {
                connectedWith = null;
                updateConnectionStatus(null);
                alert(senderName + " disconnected.");
            }
            break;
        case 'Chat':
            if (connectedWith == senderId) {
                updateChat(msg.feature, msg.payload.text, true);
            }
            break;
    }
}
