...
// private is a websocket message handler that routes messages to a specific client
func private(id, msg string) {
	targetID := gjson.Get(msg, "target").String()
	if targetID == "" {
		return
	}

	idmu.Lock()
	targetConnID, ok := clientConnM[targetID]
	idmu.Unlock()
